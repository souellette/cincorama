...
class Sm_Megamenu_Block_List extends Mage_Core_Block_Template
{
	...
	public function getItemsV(){
		//get Menu items
		$helper = Mage::helper('megamenu');
		$group_item = Mage::getModel('megamenu/menugroup')->load($this->_config['group_id_v']);
		if($group_item->getStatus() == Sm_Megamenu_Model_System_Config_Source_Status::STATUS_ENABLED){
			$collection_items = $helper ->getItemsByLv($this->_config['start_level'],$this->_config['group_id_v']);
			return $collection_items;
		}
		else{
			return array();
